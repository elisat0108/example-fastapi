...
@pytest.mark.parametrize("title, content, published",[
    ...
    ("favourite pizza new title", "I love pepperoni", False),
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "arbitrary title", "content": "asdfjsklfj"})
    
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary title"
    assert created_post.content == "asdfjsklfj"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_posts(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "arbitrary title", "content": "asdfjsklfj"})
    assert res.status_code == 401
...
def test_update_post(authorized_client, test_user, test_posts):
    data = {
        "title": "updated title", 
        "content": "updated ocontent", 
        "id": test_posts[0].id
    }
    res = authorized_client.delete(f"/posts/{test_posts[0].id}", json=data)
